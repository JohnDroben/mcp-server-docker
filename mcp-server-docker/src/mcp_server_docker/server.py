...
async def call_tool(
    ...
    try:
        if name == "list_containers":
            ...
            result = {"result": random.choice(["heads", "tails"])}
...
async def run_tcp(settings: ServerSettings, docker_client: docker.DockerClient):
    """Run the server on a TCP socket with the given settings and Docker client."""
    ...
    async with tcp_server(host="0.0.0.0", port=8080) as (read_stream, write_stream):
